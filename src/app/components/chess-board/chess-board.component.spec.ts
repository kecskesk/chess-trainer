...
const createEnterLike = (srcRow: number, srcCol: number, targetRow: number, targetCol: number) => {
  return {
    ...
    isValidMove: ChessRulesService.validateMove(
      targetRow,
      targetCol,
      chessBoardStateService.field[targetRow][targetCol],
      srcRow,
      srcCol
    ).isValid
  } as any;
...
const createCctServiceStub = (overrides: Partial<ChessBoardCctService> = {}) => ({
...
const createUiTextLoaderStub = (overrides: Partial<UiTextLoaderService> = {}) => ({
...
beforeEach(() => {
  ...
  component = new ChessBoardComponent(chessBoardStateService, {
    get: () => of([])
  } as any, createCctServiceStub(), createUiTextLoaderStub(), stockfishServiceStub as any, undefined, undefined);
...
describe('ChessBoardComponent opening recognition', () => {
  ...
  it('updates opening recognition on first half-move when opening line has one step', () => {
    ...
    expect(component.getOpeningRecognition()).toBe('Waiting for opening line...');

        movePiece(6, 3, 4, 3);
    expect(component.getOpeningRecognition()).toBe('Queen\'s Pawn Opening');
  });
  ...
  it('does not repeat suggested response after it has been played', () => {
    ...
    expect(component.getOpeningRecognition()).toBe('Queen\'s Gambit Declined');
...
describe('ChessBoardComponent coverage helpers (js flip mapping)', () => {
  it('covers display mapping and arrow remap helpers across flip states', () => {
    ...
    const normalDisplay = ChessBoardComponentUtils.getDisplayBoardPosition(0, 0, component.isBoardFlipped);
    expect(`field${normalDisplay.row}${normalDisplay.col}`).toBe('field00');
    expect(component.translateFieldNames(normalDisplay.row, normalDisplay.col)).toBe('a8');
    expect(chessBoardStateService.field[normalDisplay.row][normalDisplay.col][0]?.piece).toBe(ChessPiecesEnum.Rook);

    component.isBoardFlipped = true;
    const flippedDisplay = ChessBoardComponentUtils.getDisplayBoardPosition(0, 0, component.isBoardFlipped);
    ...
    expect(`translate(-50%, -50%) rotate(${ChessBoardDisplayUtils.mapRotationForDisplay('390deg', component.isBoardFlipped)})`)
      .toBe('translate(-50%, -50%) rotate(210deg)');

    expect(ChessBoardDisplayUtils.mapPercentCoordinateForDisplay('calc(20%)', component.isBoardFlipped)).toBe('calc(20%)');
    expect(`translate(-50%, -50%) rotate(${ChessBoardDisplayUtils.mapRotationForDisplay('angle(10)', component.isBoardFlipped)})`)
      .toBe('translate(-50%, -50%) rotate(angle(10))');
    expect(ChessBoardDisplayUtils.mapPercentCoordinateForDisplay('', component.isBoardFlipped)).toBe('');
    expect(`translate(-50%, -50%) rotate(${ChessBoardDisplayUtils.mapRotationForDisplay('', component.isBoardFlipped)})`)
      .toBe('translate(-50%, -50%) rotate()');
  });
...
describe('ChessBoardComponent gameplay moves and rules (drag preview and clock state)', () => {
  ...
  it('reads check highlight through isCheck helper', () => {
    ...
    expect(!!chessBoardStateService.boardHelper.checks['33']).toBeTrue();
    expect(!!chessBoardStateService.boardHelper.checks['22']).toBeFalse();
  });
...
describe('ChessBoardComponent gameplay moves and rules (clock and controls)', () => {
  ...
  it('supports undo and redo navigation over visible history', () => {
    chessBoardStateService.boardHelper.history = {
      '1': 'e2-e4',
      '2': 'e7-e5',
      '3': 'Ng1-f3'
  ...
  it('restores castling rights through undo and redo over a king move', () => {
    expect(ChessRulesService.getCastlingRightsNotation(
...
describe('ChessBoardComponent gameplay moves and rules (clock and controls promotion)', () => {
  beforeEach(() => {
    chessBoardStateService = new ChessBoardStateService();
    component = new ChessBoardComponent(chessBoardStateService, { get: () => of([]) } as any, createCctServiceStub() as any, createUiTextLoaderStub() as any, stockfishServiceStub as any, undefined, undefined);
    chessBoardStateService.boardHelper.colorTurn = ChessColorsEnum.White;
  });
...
describe('ChessBoardComponent gameplay moves and rules (clock and controls continued)', () => {
  ...
  it('returns castling rights notation from board state', () => {
  ...
  it('produces export and annotation helper outputs', () => {
    ...
    component.exportBoardImage();
    expect(chessBoardStateService.boardHelper.debugText).toContain('Export: Board image ready');
...
describe('ChessBoardComponent gameplay moves and rules (clock and controls overlays)', () => {
  ...
  it('covers DOM image export and download helper paths', async () => {
    ...
    await component.exportBoardImage();
    ...
    const formatted = ChessBoardOpeningUtils.formatOpeningDebugText(
      {
        ...
        steps: ['e2-e4'],
        raw: {
          suggested_best_response_name: 'Sample Response',
          suggested_best_response_notation_step: '... c7-c5',
          short_description: 'opening notes'
        }
      } as any,
      0,
      0,
...
describe('ChessBoardComponent gameplay moves and rules (position and analysis)', () => {
  ...
  it('returns opening and endgame defaults', () => {
    (component as any).openingsLoaded = false;
    expect(component.getOpeningRecognition()).toBe('Waiting for opening line...');
    expect(component.getEndgameRecognition()).toBe('Not endgame yet');
  });
...
describe('ChessBoardComponent gameplay moves and rules (time and overlays)', () => {
  ...
  it('covers clock and board-orientation helper branches', () => {
    ...
    component.isBoardFlipped = true;
    const flipped = ChessBoardComponentUtils.getDisplayBoardPosition(0, 0, component.isBoardFlipped);
    expect(((flipped.row + flipped.col) % 2) === 0).toBeTrue();
  });
...
describe('ChessBoardComponent gameplay moves and rules (startup and loading)', () => {
  it('covers openings payload mapping and loadOpenings error callback', () => {
    ...
    } as any, createCctServiceStub() as any, createUiTextLoaderStub() as any, stockfishServiceStub as any, undefined, undefined);
...
describe('ChessBoardComponent branch coverage helpers (guard and fallback paths)', () => {
  ...
  it('covers formatClock sub-minute formatting and opening loading/no-match branches', () => {
    expect(ChessBoardClockUtils.formatClock(5900)).toContain('.');
    (component as any).openingsLoaded = false;
    chessBoardStateService.boardHelper.history = { '1': 'e2-e4' } as any;
    expect(component.getOpeningRecognition()).toBe('Loading openings...');

    (component as any).openingsLoaded = true;
    (component as any).activeOpening = null;
    expect(component.getOpeningRecognition()).toBe('No opening match');
  });
  ...
  it('covers endgame transition branch and black suggested moves branch', () => {
    ...
    expect(component.getEndgameRecognition()).toBe('Transition phase');

    chessBoardStateService.boardHelper.colorTurn = ChessColorsEnum.Black;
    expect(component.getSuggestedMoves()[0]).toContain('...');
  });
...
describe('ChessBoardComponent branch coverage helpers (private helpers)', () => {
  it('uses chessBoardCctService for recommendations', () => {
    const cctSpy = jasmine.createSpy('ensureCctRecommendations').and.returnValue({
      [CctCategoryEnum.Captures]: [{ move: 'Qh5+', tooltip: 'x' }],
      [CctCategoryEnum.Checks]: [],
      [CctCategoryEnum.Threats]: []
    });
    (component as any).chessBoardCctService = { ensureCctRecommendations: cctSpy } as any;

    expect(component.getCctRecommendations(CctCategoryEnum.Captures)).toEqual([{ move: 'Qh5+', tooltip: 'x' }]);
    expect(cctSpy).toHaveBeenCalled();
  });
...
describe('ChessBoardComponent branch coverage helpers (drag and drop guards)', () => {
  it('covers drop-enter, drag-start and pointer guard branches', () => {
    ...
    component.onDropListEntered({ sourceRow: 1, sourceCol: 1, targetRow: 2, targetCol: 2, isValidMove: false } as any);
    component.onDropListEntered({ sourceRow: 9, sourceCol: 0, targetRow: 0, targetCol: 0, isValidMove: false } as any);
...
describe('ChessBoardComponent branch coverage helpers (opening recognition edge paths)', () => {
  it('covers opening tie-break branch for shorter complete line preference', () => {
    ...
    component.getOpeningRecognition();
    expect(component.getOpeningRecognition().length).toBeGreaterThan(0);
  });
...
describe('ChessBoardComponent branch coverage helpers (simulation and clock branches)', () => {
  ...
  it('covers clock start/tick/increment and forfeit early-return branches', () => {
    const zoneComponent = new ChessBoardComponent(
      chessBoardStateService,
      { get: () => of([]) } as any,
      createCctServiceStub() as any,
      createUiTextLoaderStub() as any,
...
describe('ChessBoardComponent branch coverage helpers (index and evaluation branches)', () => {
  it('covers move-index helpers, notation rule branches and insufficient-material terminal false', () => {
    ...
    expect(ChessBoardHistoryService.getCurrentVisibleMoveIndex((component as any).getMaxMoveIndex(), component.historyCursor)).toBe(-1);
    chessBoardStateService.boardHelper.history = { '1': 'e2-e4' } as any;
    component.historyCursor = 5;
    expect(ChessBoardHistoryService.getCurrentVisibleMoveIndex((component as any).getMaxMoveIndex(), component.historyCursor)).toBe(0);
...
describe('ChessBoardComponent stockfish evaluation thresholds', () => {
  ...
  it('covers evaluation helpers and analysis meter branches', () => {
    ...
    const noEngineComponent = new ChessBoardComponent(chessBoardStateService, { get: () => of([]) } as any, createCctServiceStub() as any, createUiTextLoaderStub() as any, stockfishServiceStub as any, undefined, undefined);
...
describe('ChessBoardComponent branch coverage helpers (locale switching and opening assets)', () => {
  ...
  it('covers opening-load stale callbacks and opening-file fallback catches', () => {
    ...
    const localComponent = new ChessBoardComponent(chessBoardStateService, httpStub as any, createCctServiceStub() as any, createUiTextLoaderStub() as any, stockfishServiceStub as any, undefined, undefined);
    (localComponent as any).loadOpeningsFromAssets('');

    const firstPath = String(httpStub.get.calls.argsFor(0)[0]);
...
describe('ChessBoardComponent preview presets and render slices', () => {
  it('covers preview slicing and piece-colors preset mapping branches', () => {
    ...
    expect(ChessBoardComponentUtils.getPieceColorPreviewCell(0, 0, component.renderedBoardRows, component.renderedBoardCols)[0]).toEqual(jasmine.objectContaining({
      color: ChessColorsEnum.White,
      piece: ChessPiecesEnum.Rook
    }));
    expect(ChessBoardComponentUtils.getPieceColorPreviewCell(0, 1, component.renderedBoardRows, component.renderedBoardCols)[0]).toEqual(jasmine.objectContaining({
      color: ChessColorsEnum.Black,
      piece: ChessPiecesEnum.Bishop
    }));
    expect(ChessBoardComponentUtils.getPieceColorPreviewCell(1, 0, component.renderedBoardRows, component.renderedBoardCols)[0]).toEqual(jasmine.objectContaining({
      color: ChessColorsEnum.White,
      piece: ChessPiecesEnum.Pawn
    }));
    expect(ChessBoardComponentUtils.getPieceColorPreviewCell(1, 1, component.renderedBoardRows, component.renderedBoardCols)[0]).toEqual(jasmine.objectContaining({
      color: ChessColorsEnum.White,
...
describe('ChessBoardComponent suggestion scoring helpers basics', () => {
  it('covers suggestion getters and white suggested branch', () => {
    chessBoardStateService.boardHelper.colorTurn = ChessColorsEnum.White;
    expect(component.getSuggestedMoves()).toEqual(['Qh5+', 'Nxe5', 'd4']);
...
describe('ChessBoardComponent additional suggestion coverage', () => {
  ...
  it('covers refreshSuggestedMoves callback branch with explicit move arrays', async () => {
    const local = new ChessBoardComponent(chessBoardStateService, { get: () => of([]) } as any, createCctServiceStub() as any, createUiTextLoaderStub() as any, {
...
describe('ChessBoardComponent additional suggestion coverage (preview helpers)', () => {
  ...
  it('covers residual board component branches for coverage', () => {
    ...
    const localWithNoHistory = new ChessBoardComponent(new ChessBoardStateService(), { get: () => of([]) } as any, createCctServiceStub() as any, createUiTextLoaderStub() as any, stockfishServiceStub as any, undefined, undefined);
    ...
    expect(() =>
      (ChessBoardComponent.prototype as any).appendCheckmateResultIfNeeded.call({ chessBoardStateService: null })
    ).not.toThrow();

    const appendSpy = jasmine.createSpy('append');
    (ChessBoardComponent.prototype as any).appendCheckmateResultIfNeeded.call({
      chessBoardStateService: { boardHelper: { checkmateColor: ChessColorsEnum.Black } },
      uiText: { status: { checkmatePrefix: '' } },
